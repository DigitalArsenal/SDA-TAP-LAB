...
HeadingPitchRoll.fromQuaternion = function(quaternion, result, referenceFrame) {
  if (!defined_default(quaternion)) {
  ...
  if (defined_default(referenceFrame)) {
    if (typeof referenceFrame === "function") {
      frame = referenceFrame();
    } else if (referenceFrame instanceof Matrix4_default) {
      frame = referenceFrame;
    } else {
      throw new DeveloperError_default(
        "referenceFrame must be a Matrix4 or a function returning a Matrix4"
      );
    }
    const inReferenceFrame = Matrix4_default.inverse(frame, new Matrix4_default());
    const inRotation = Matrix4_default.getRotation(inReferenceFrame, new Matrix3_default());
    const inRotationQuaternion = Quaternion_default.fromRotationMatrix(
      inRotation,
      new Quaternion_default()
    );
    localQuaternion = Quaternion_default.multiply(
      inRotationQuaternion,
      quaternion,
      new Quaternion_default()
    );
  } else {
  ...
  const test = 2 * (localQuaternion.w * localQuaternion.y - localQuaternion.z * localQuaternion.x);
  const denominatorRoll = 1 - 2 * (localQuaternion.x * localQuaternion.x + localQuaternion.y * localQuaternion.y);
  const numeratorRoll = 2 * (localQuaternion.w * localQuaternion.x + localQuaternion.y * localQuaternion.z);
  const denominatorHeading = 1 - 2 * (localQuaternion.y * localQuaternion.y + localQuaternion.z * localQuaternion.z);
  const numeratorHeading = 2 * (localQuaternion.w * localQuaternion.z + localQuaternion.x * localQuaternion.y);
...
// packages/engine/Source/DataSources/ExternalData.js
var fileSystem = {};
...
function buildModuleUrl(relativeUrl) {
  if (fileSystem[relativeUrl]) {
    return fileSystem[relativeUrl];
  }
...
function Iau2006XysData(options) {
  ...
  this._samples = XYS2006_samples || new Array(this._totalSamples * 3);
...
// packages/engine/Source/Core/ReferenceFrame.js
var ReferenceFrame = {
  /**
   * Earth-Centered-Earth-Fixed (ECEF) frame:
   *
   * In the ECEF frame, the origin is at the Earth's center, and it rotates along with the Earth.
   * This fixed frame has its axes oriented with respect to the Earth's surface.
   * - The X-axis points toward the prime meridian (intersection of the equator and prime meridian).
   * - The Y-axis points 90 degrees east of the prime meridian.
   * - The Z-axis points towards the North Pole.
   *
   * ECEF is useful for representing points on or near the Earth's surface.
   *
   * @type {number}
   * @constant
   */
  FIXED: 0,
  /**
   * The International Celestial Reference Frame (ICRF). An inertial frame of reference
   * that doesn't rotate with respect to distant stars. It's based on the positions of quasars,
   * which are extremely distant and serve as essentially fixed points in space. This frame
   * is used for high-precision astronomy and astrometry, providing a stable frame of reference
   * that isn't affected by Earth's rotation or other planetary movements.
   *
   * @type {number}
   * @constant
   */
  ...
  /**
   *
   * The True Equator Mean Equinox (TEME) frame.
   * TEME is a reference frame used in the Simplified General Perturbations 4 (SGP4) model
   * for predicting the positions of objects in Earth orbit. It is a dynamically propagated
   * reference frame that takes into account Earth's precession and nutation.
   *
   * @type {number}
   * @constant
   */
  TEME: 2,
  /**
   * The Vehicle Velocity, Local Horizontal (VVLH) frame or North-up from deep space:
   *
   * This frame is computed based on the position and velocity of the entity.
   * It's an approximation used for objects in motion, especially satellites.
   * - Z-axis points along the position (from the center of the Earth towards the entity's current position).
   * - Y-axis points in the direction of the angular momentum vector (orbit normal).
   * - X-axis points in the direction of motion (right handed basis).
   *
   * It's utilized for fast-moving objects like satellites.
   *
   * @type {number}
   * @constant
   */
   ...
   *
   * A local tangent plane coordinate system used for near-surface entities.
   * - The **East** axis points in the direction of the local geographic east.
   * - The **North** axis points in the direction of the local geographic north.
   * - The **Up** axis points away from the center of the body (typically Earth) and is perpendicular to the plane formed by the East and North axes.
   *
   * ENU is used for objects that are stationary, slow-moving, or at low altitudes. It provides a natural "top-down" view for ground-based entities.
   *
   * @type {number}
   * @constant
   */
   ...
   *
   * A local tangent plane coordinate system commonly used in aviation, marine, and ground applications.
   * - The **North** axis points in the direction of the local geographic north.
   * - The **East** axis points in the direction of the local geographic east.
   * - The **Down** axis points towards the center of the body (typically Earth) and is perpendicular to the plane formed by the North and East axes.
   *
   * NED is often used for aircraft and other fast-moving vehicles that operate at various altitudes, and aligns with Azimuth / Elevation for ground sites.
   * It provides an intuitive frame of reference where the 'Down' axis is aligned with gravity, and 'North' and 'East' align with their respective geographic directions.
   *
   * @type {number}
   * @constant
   */
  NED: 5,
  /**
   * North-East-Up (NEU) frame:
   *
   * A local tangent plane coordinate system similar to NED but with a different orientation for the vertical axis.
   * - The **North** axis points in the direction of the local geographic north.
   * - The **East** axis points in the direction of the local geographic east.
   * - The **Up** axis points away from the center of the body (typically Earth) and is perpendicular to the plane formed by the North and East axes.
   *
   * NEU is a variant of the NED coordinate system where the 'Up' axis is aligned opposite to gravity, and 'North' and 'East' align with their respective geographic directions.
   * It may be useful in contexts where a more conventional 'Up' direction is desired, as opposed to 'Down' towards the center of the Earth.
   *
   * @type {number}
   * @constant
   */
   ...
   *
   * This frame is aligned with the UVW coordinate system, specific for spacecraft, and has the following characteristics:
   ...
   *
   * Note: The RIC coordinate system is aligned with the UVW coordinate system, but it's not considered quasi-inertial.
   * It's not treated as an inertial coordinate system for the purposes of velocity vector conversions.
   *
   * @type {number}
   * @constant
   */
...
var reverseHeading = [ReferenceFrame.NEU];
...
Transforms.localToFixedFrameQuaternion = function(origin, localQuaternion, ellipsoid, fixedFrameTransform, result) {
  if (!defined_default(localQuaternion)) {
    throw new DeveloperError_default("localQuaternion is required");
  }
  fixedFrameTransform = defaultValue_default(
    fixedFrameTransform,
    Transforms.eastNorthUpToFixedFrame
  );
  const fixedFrameMatrix = fixedFrameTransform(origin, ellipsoid);
  const rotationMatrix = Matrix4_default.getMatrix3(fixedFrameMatrix, new Matrix3_default());
  const fixedFrameQuaternion = Quaternion_default.fromRotationMatrix(rotationMatrix);
  result = defined_default(result) ? result : new Quaternion_default();
  return Quaternion_default.multiply(fixedFrameQuaternion, localQuaternion, result);
};
...
var gltfPosScratch1 = new Cartesian3_default();
var gltfPosScratch2 = new Cartesian3_default();
var gltfPosScratch3 = new Cartesian3_default();
var gltfPosScratch4 = new Cartesian3_default();
var gltfPosScratch5 = new Cartesian3_default();
var gltfPosScratch6 = new Cartesian3_default();
var gltfPosScratch7 = new Cartesian3_default();
var gltfPosScratch8 = new Cartesian3_default();
var deltaTime = new JulianDate_default();
var gltfMatrixScratch1 = new Matrix3_default();
var gltfMatrixScratch2 = new Matrix3_default();
var gltfMatrixScratch3 = new Matrix3_default();
Transforms.vvlhToFixed = function(position, time, result) {
  const cartesian = position.getValue(time, gltfPosScratch1);
  JulianDate_default.addSeconds(time, 0.01, deltaTime);
  const deltaCartesian = position.getValue(deltaTime, gltfPosScratch2);
  const ellipsoid = Ellipsoid_default.WGS84;
  const maxRadii = ellipsoid.maximumRadius + 1e5;
  if (!defined_default(result)) {
    result = new Matrix4_default();
  }
  if (Cartesian3_default.magnitude(cartesian) <= maxRadii) {
    const velocity = Cartesian3_default.subtract(
      deltaCartesian,
      cartesian,
      new Cartesian3_default()
    );
    let east;
    if (Cartesian3_default.magnitude(velocity) === 0) {
      const enuMatrix = Transforms.eastNorthUpToFixedFrame(
        cartesian,
        ellipsoid
      );
      east = new Cartesian3_default(enuMatrix[0], enuMatrix[1], enuMatrix[2]);
    } else {
      east = Cartesian3_default.normalize(velocity, new Cartesian3_default());
    }
    const up = ellipsoid.geodeticSurfaceNormal(cartesian, new Cartesian3_default());
    const north = Cartesian3_default.cross(up, east, new Cartesian3_default());
    result[0] = east.x;
    result[1] = east.y;
    result[2] = east.z;
    result[3] = 0;
  ...
  const toInertial = Transforms.computeFixedToIcrfMatrix(
    time,
    gltfMatrixScratch1
  );
  if (!toInertial) {
    return new Matrix4_default();
  }
  const toInertialDelta = Transforms.computeFixedToIcrfMatrix(
    deltaTime,
    gltfMatrixScratch2
  );
  const toFixed = Matrix3_default.transpose(toInertial, gltfMatrixScratch3);
  ...
  if (!Cartesian3_default.equalsEpsilon(
    ...
    const velocityVector = Cartesian3_default.subtract(
      inertialDeltaCartesian,
      inertialCartesian,
      gltfPosScratch5
    );
    const zBasis = gltfPosScratch6;
    try {
      ...
      console.log(inertialCartesian, zBasis);
    }
    const yBasis = Cartesian3_default.cross(velocityVector, zBasis, gltfPosScratch7);
    Cartesian3_default.negate(yBasis, yBasis);
    const xBasis = Cartesian3_default.cross(yBasis, zBasis, gltfPosScratch8);
    try {
...
Transforms.reverseHeadingQuaternion = function(orientation) {
  if (!defined_default(orientation)) {
    throw new DeveloperError_default("orientation is required");
  }
  const hpr = new HeadingPitchRoll_default();
  HeadingPitchRoll_default.fromQuaternion(orientation, hpr);
  hpr.heading = -hpr.heading;
  hpr.roll = -hpr.roll;
  const newOrientation = new Quaternion_default();
  Quaternion_default.fromHeadingPitchRoll(hpr, newOrientation);
  return newOrientation;
};
var yPositive90Matrix4 = new Matrix4_default(
  0,
  0,
  1,
  0,
...
Transforms.getPositionInVVLH = function(referenceEntity, targetEntity, time, currentTime, referenceFrame = ReferenceFrame_default.FIXED) {
  const targetPosition = targetEntity.position.getValueInReferenceFrame(
    time,
    referenceFrame.FIXED
  );
  const currentReferencePosition = referenceEntity.position.getValueInReferenceFrame(
    currentTime,
    referenceFrame.FIXED
  );
  let fixedToVvlhTransform;
  try {
    fixedToVvlhTransform = Matrix4_default.inverse(
      Transforms.vvlhToFixed(referenceEntity.position, time),
      new Matrix4_default()
    );
  } catch (e) {
    fixedToVvlhTransform = new Matrix4_default();
  }
  const targetInVVLH = Matrix4_default.multiplyByPoint(
    fixedToVvlhTransform,
    targetPosition,
    new Cartesian3_default()
  );
  const currentRotation = Matrix4_default.getRotation(
    Transforms.vvlhToFixed(referenceEntity.position, currentTime),
    new Matrix3_default()
  );
  const rotatedTarget = Matrix3_default.multiplyByVector(
    currentRotation,
    targetInVVLH,
    new Cartesian3_default()
  );
  let finalPosition = Cartesian3_default.add(
    currentReferencePosition,
    rotatedTarget,
    new Cartesian3_default()
  );
  if (referenceFrame === ReferenceFrame_default.INERTIAL) {
    const fixedToInertial = Transforms.computeFixedToIcrfMatrix(currentTime);
    if (fixedToInertial) {
...
var nwTransform = Transforms.localFrameToFixedFrameGenerator("north", "west");
Transforms.getTransformToFixedFrame = function(referenceFrame, time, position) {
  let transformToFixedFrame;
  const _referenceFrameTransforms = {
    [ReferenceFrame_default.FIXED]: () => Matrix4_default.IDENTITY,
    [ReferenceFrame_default.INERTIAL]: () => {
      if (!defined_default(time)) {
        throw new DeveloperError_default(
          "Conversion from INERTIAL to FIXED requires a time."
        );
      }
      return Transforms.computeIcrfToFixedMatrix(time);
    },
    [ReferenceFrame_default.TEME]: () => {
      throw new DeveloperError_default(
        "Conversion from TEME to FIXED is not implemented."
      );
    },
    [ReferenceFrame_default.VVLH]: () => {
      if (!defined_default(time) || !defined_default(position)) {
        throw new DeveloperError_default(
          "Conversion from VVLH to FIXED requires both time and position."
        );
      }
      const transform = Transforms.vvlhToFixed(position, time);
      const axisFlip = Matrix4_default.fromScale(new Cartesian3_default(1, -1, -1));
      return Matrix4_default.multiply(transform, axisFlip, new Matrix4_default());
    },
    [ReferenceFrame_default.NED]: () => {
      if (!defined_default(time) || !defined_default(position)) {
        throw new DeveloperError_default(
          "Conversion from NED to FIXED requires both time and position."
        );
      }
      const transform = nwTransform(position.getValue(time));
      const axisFlip = Matrix4_default.fromScale(new Cartesian3_default(1, -1, -1));
      return Matrix4_default.multiply(transform, axisFlip, new Matrix4_default());
    },
    [ReferenceFrame_default.NEU]: () => {
      if (!defined_default(time) || !defined_default(position)) {
        throw new DeveloperError_default(
          "Conversion from NEU to FIXED requires both time and position."
        );
      }
      const transform = nwTransform(position.getValue(time));
      const axisFlip = Matrix4_default.fromScale(new Cartesian3_default(1, -1, 1));
    ...
    [ReferenceFrame_default.RIC]: () => {
      if (!defined_default(time) || !defined_default(position)) {
        throw new DeveloperError_default(
          "Conversion from RIC to FIXED requires both time and position."
        );
      }
      const transform = Transforms.vvlhToFixed(position, time);
      const axisReorder = Matrix4_default.fromArray([
        0,
        0,
        1,
        0,
      ...
      return Matrix4_default.multiply(transform, axisReorder, new Matrix4_default());
    }
  };
  if (_referenceFrameTransforms.hasOwnProperty(referenceFrame)) {
    transformToFixedFrame = _referenceFrameTransforms[referenceFrame];
  } else {
    transformToFixedFrame = Transforms.eastNorthUpToFixedFrame;
  }
  return transformToFixedFrame;
};
