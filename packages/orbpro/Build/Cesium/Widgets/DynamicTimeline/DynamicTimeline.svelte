<script>
  ...
  let dragging, offset;
  let getDate = (d) => (d ? JulianDate.toDate(d) : new Date());
  let fromDate = (d) => (d ? JulianDate.fromDate(d) : new JulianDate());
  let dWidth = 0;
  let baseNumTicks = 60;
  let numTicks = 60;
  $: quarterTick = (n) => numTicks / 4;
  ...
  let tickWidth = (t) => parseFloat(((dWidth / numTicks) * t).toFixed(2));
  let tickTime = (i, xpos) => {
    let _displayTime = new JulianDate();
  ...
  export function getOffsetX(evt) {
  ...
  export function syncShuttlePosition(xposition, xtime) {
    ...
    let _xpercent = _timeDelta / _range;
  }
