<script lang="ts">
  ...
  function setValue(val: any, e?: any) {
    // Adjust the value to adhere to the step increments
  ...
  function calculateNewValue(clientX: any, e: any) {
    let delta = clientX - (elementX + 10);
    let percent = (delta * 100) / (container.clientWidth - 10);
    percent = Math.max(0, Math.min(percent, 100)); // Ensuring percent is between 0 and 100

    // Calculate raw value based on percent
    let rawValue = (percent * (max - min)) / 100 + min;

    // Adjusting rawValue to the nearest step
    let steppedValue = Math.round(rawValue / step) * step;

    // Ensuring steppedValue is within min and max bounds
    steppedValue = Math.max(min, Math.min(steppedValue, max));

    setValue(steppedValue, e);
  }
  ...
  // Add mouseup listener to window to handle release outside of component
