...
{#if minimized}
  <button
    class="rounded fixed flex items-center justify-center top-12 right-[5px] pr-3 cesium-button"
    on:click={() => {
      minimized = false;
    }}><Icon scale={1} data={chevronLeft} /></button
  >
{:else}
  <div
    class="fixed mt-12 pr-2 pl-2 flex justify-end items-start w-full md:w-[38%] h-[25%] md:h-[34%] max-h-[300px] right-0"
  >
