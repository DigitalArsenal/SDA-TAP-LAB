...
const siteTypeColors: { [key: string]: Color } = {
    ...
    "1": Color.LIMEGREEN, // Example color for site type 1
...
export const processSIT = async (viewer: Viewer, data: ExtendedSITCOLLECTIONT) => {
    ...
    const addEntity = (record: ExtendedSITT) => {
        return new Promise<void>((resolve, reject) => {
            ...
            try {
                position = Cartesian3.fromDegrees(parseFloat(record.LONGITUDE.toString()), parseFloat(record.LATITUDE.toString()), parseFloat(record.ALTITUDE.toString()) || 20);
            } catch (e) {
            ...
            //@ts-ignore
            if (siteColor !== Color.WHITE || (record?.NAME || "").indexOf("SDA") as any > -1) {
